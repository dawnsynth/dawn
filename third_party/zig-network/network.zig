...
const WindowsOSLogic = struct {
    ...
    const FdSet = extern struct {
        ...
        fn fdSlice(self: *align(8) FdSet) []windows.ws2_32.SOCKET {
            const ptr: [*]u8 = @ptrCast(@alignCast(self));
        ...
        fn memSlice(self: *align(8) FdSet) []u8 {
            const ptr: [*]u8 align(8) = @ptrCast(@alignCast(self));
            return @alignCast(ptr[0..(4 * @sizeOf(c_uint) + self.capacity * @sizeOf(windows.ws2_32.SOCKET))]);
        }

        fn deinit(self: *align(8) FdSet, allocator: std.mem.Allocator) void {
            //@compileLog(@typeInfo(@TypeOf(self)));
            const ptr: []u8 align(8) = @alignCast(self.memSlice());
            allocator.free(ptr);
        }
        ...
        fn addFd(fd_set: **align(8) FdSet, allocator: std.mem.Allocator, new_fd: windows.ws2_32.SOCKET) !void {
            if (fd_set.*.size == fd_set.*.capacity) {
                ...
                std.debug.print("info {}", .{@typeInfo(@TypeOf(ptr))});
                const tmp: []u8 align(8) = @alignCast(try allocator.reallocAdvanced(ptr, new_mem_size, @returnAddress()));
                const tmp_ptr: [*]u8 align(8) = @alignCast(tmp.ptr);
                if (std.mem.isAligned(@intFromPtr(tmp_ptr), 6)) {
                    unreachable;
                }
                fd_set.* = @ptrCast(@alignCast(tmp_ptr));
                fd_set.*.capacity *= 2;
            }
